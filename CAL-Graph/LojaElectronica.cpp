...
void LojaElectronica::addZonaGrafo(Zona* z1) {
	...
	do {
		...
		string desig;
		int dist;
		cout << "Insira a designacao de uma Zona � qual ligar: ";
		fflush(stdin);
		getline(cin,desig);
		...
		cout << "Deseja inserir mais alguma dist�ncia a outra zona? (S/N): ";
		fflush(stdin);
		cin >> resp;
	} while(resp!='n' || resp != 'N');
}
...
void LojaElectronica::addLoja()
{
	...
	Zona *zonaLoja=procuraZona(zona); //TODO excep��o
...
void LojaElectronica::removeLoja(unsigned int codLoja)
{
	...
	if(encontrou==false){
		...
		throw Excepcao("\n N鉶 existe nenhuma loja com esse ID \n");
